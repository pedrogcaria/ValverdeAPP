...
describe('calculateMetrics', () => {
  it('exclui reservas canceladas e separa receitas recebidas', () => {
    const metrics = calculateMetrics([reservation({ payment_status: 'paid' }), reservation({ id: 'cancelled', status: 'cancelled', total_amount: 500 })], [expense({ amount: 140 })]);
    expect(metrics).toMatchObject({ grossRevenue: 1000, commissions: 100, expenses: 140, netRevenue: 760, paidRevenue: 1000, openBalance: 0, activeReservations: 1 });
  });
});

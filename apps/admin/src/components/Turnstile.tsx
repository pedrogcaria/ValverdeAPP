...
type Props = { action: 'login' | 'password_reset'; onTokenChange: (token: string | null) => void };

export function Turnstile({ action, onTokenChange }: Props) {
  ...
  useEffect(() => {
    ...
    loadTurnstile().then((turnstile) => turnstile.ready(() => {
      ...
      widgetId.current = turnstile.render(container.current, {
        ...
        callback: (token: string) => { setMessage(undefined); onTokenChange(token); },
    ...
    return () => { disposed = true; if (widgetId.current && window.turnstile) window.turnstile.remove(widgetId.current); };
  }, [action, onTokenChange, siteKey]);

  return <div className="turnstile"><div ref={container} />{message && <p className="form-help form-help--warn">{message}</p>}</div>;
}

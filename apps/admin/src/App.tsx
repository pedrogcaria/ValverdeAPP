...
import { appError, EXPENSES_SELECT } from './lib/data-access';
...
function AdminShell({ session }: { session: Session }) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const [settingsResult, guestsResult, reservationsResult, expensesResult, requestsResult, ratesResult, promosResult] = await Promise.all([
        db.from('property_settings').select('*').maybeSingle(),
        db.from('guests').select('*').order('full_name'),
        db.from('reservations').select('*, guests(id, full_name, email, phone, nationality)').order('check_in', { ascending: true }),
        db.from('expenses').select(EXPENSES_SELECT).order('occurred_on', { ascending: false }),
        db.from('booking_requests').select('*').order('created_at', { ascending: false }),
        db.from('seasonal_rates').select('*').order('starts_on'),
        db.from('promo_codes').select('*').order('code')
      ]);
      const results = [settingsResult, guestsResult, reservationsResult, expensesResult, requestsResult, ratesResult, promosResult];
      ...
      setData({
        ...
        requests: (requestsResult.data ?? []) as BookingRequest[],
        rates: (ratesResult.data ?? []) as SeasonalRate[],
        promoCodes: (promosResult.data ?? []) as PromoCode[]
      });
  ...
  const acceptRequest = async (request: BookingRequest) => {
    ...
    if (hasDateConflict(data.reservations, request.check_in, request.check_out)) {
      tell('error', 'Há uma reserva ativa sobreposta. Confirme as datas antes de aceitar este pedido.'); return;
    }
    if (!window.confirm(`Criar uma reserva pendente para ${request.full_name}?`)) return;
    try {
      const db = getSupabase();
      const existingGuest = data.guests.find((guest) => guest.email?.toLowerCase() === request.email.toLowerCase() || normalizePhone(guest.phone) === normalizePhone(request.phone));
      let guestId = existingGuest?.id;
      if (existingGuest) {
        const { error: guestError } = await db.from('guests').update({ full_name: request.full_name, email: request.email, phone: request.phone }).eq('id', existingGuest.id);
        if (guestError) throw guestError;
      } else {
        const { data: guest, error: guestError } = await db.from('guests').insert({ owner_id: ownerId, full_name: request.full_name, email: request.email, phone: request.phone }).select('id').single();
        if (guestError || !guest) throw guestError ?? new Error('Não foi possível criar o hóspede.');
        guestId = guest.id;
      }
      const { data: reservation, error: reservationError } = await db.from('reservations').insert({
        owner_id: ownerId, guest_id: guestId, source: 'site', channel: 'Villa Valverde', check_in: request.check_in, check_out: request.check_out,
        guests_count: request.guests_count, total_amount: Number(request.estimated_total_amount), payment_status: 'not_paid', status: 'pending',
        private_notes: request.guest_message, special_requests: request.heated_pool ? ['Piscina aquecida'] : []
      }).select('id').single();
      if (reservationError || !reservation) throw reservationError ?? new Error('Não foi possível criar a reserva.');
      const { error: requestError } = await db.from('booking_requests').update({ status: 'accepted', reservation_id: reservation.id }).eq('id', request.id);
...
function ReportsScreen({ data, currency, onExport }: { data: AdminData; currency: string; onExport: () => void }) {
  ...
  const rows = Object.entries(data.reservations.filter((reservation) => reservation.status !== 'cancelled').reduce<Record<string, { revenue: number; commissions: number; reservations: number }>>((accumulator, reservation) => {
    ...
    row.revenue += Number(reservation.total_amount ?? 0); row.commissions += Number(reservation.commission_amount ?? 0); row.reservations += 1;
    accumulator[key] = row; return accumulator;
  }, {})).sort(([left], [right]) => left.localeCompare(right));
  return <section className="page-content report-print"><div className="page-intro"><div><h2>Relatórios</h2><p>Receita, comissão e operação por período.</p></div><div className="row-actions"><button className="button button--quiet" onClick={onExport}><Download size={16} />Backup JSON</button><button className="button button--dark" onClick={() => window.print()}>Imprimir</button></div></div><div className="metric-strip"><Metric label="Receita bruta" value={money(metrics.grossRevenue, currency)} /><Metric label="Comissões" value={money(metrics.commissions, currency)} /><Metric label="Gastos" value={money(metrics.expenses, currency)} /><Metric label="Resultado líquido" value={money(metrics.netRevenue, currency)} emphasis /></div><section className="panel report-table"><div className="panel__heading"><div><p className="eyebrow">Por mês de check-in</p><h3>Desempenho</h3></div></div>{rows.length ? <table><thead><tr><th>Mês</th><th>Reservas</th><th>Receita</th><th>Comissões</th><th>Líquido antes de gastos</th></tr></thead><tbody>{rows.map(([month, row]) => <tr key={month}><td>{month === 'Sem data' ? month : new Intl.DateTimeFormat('pt-PT', { month: 'long', year: 'numeric' }).format(new Date(`${month}-01T00:00:00`))}</td><td>{row.reservations}</td><td>{money(row.revenue, currency)}</td><td>{money(row.commissions, currency)}</td><td>{money(row.revenue - row.commissions, currency)}</td></tr>)}</tbody></table> : <EmptyState text="Os relatórios aparecem quando existirem reservas." />}</section></section>;
}
...
function SettingsScreen({ data, ownerId, currency, onEditRate, onEditPromo, onNewRate, onNewPromo, onReload, onNotice }: { data: AdminData; ownerId: string; currency: string; onEditRate: (value: SeasonalRate) => void; onEditPromo: (value: PromoCode) => void; onNewRate: () => void; onNewPromo: () => void; onReload: () => Promise<void>; onNotice: (tone: ToastTone, message: string) => void }) {
  ...
  const importLegacy = async () => {
    if (!window.confirm('Vai criar um snapshot de valverde_data e copiar reservas, hóspedes e gastos. A tabela antiga não será alterada. Continuar?')) return;
    setSaving(true);
    const { data: result, error } = await getSupabase().rpc('import_legacy_valverde_data', { p_owner_id: ownerId });
    setSaving(false);
    if (error) { onNotice('error', error.message); return; }

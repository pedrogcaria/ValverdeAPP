...
Monorepo com duas aplicações Vite:

- `apps/site`: site público para `villavalverde.pt`.
- `apps/admin`: gestão privada para `app.villavalverde.pt`.
- `supabase`: migrações, funções e documentação da base de dados.

## Desenvolvimento

1. Copiar os ficheiros `.env.example` de cada aplicação para `.env.local` e preencher apenas os valores públicos.
...
npm run dev --workspace=@valverde/site -- --host 127.0.0.1

# Terminal 2 — gestão: http://127.0.0.1:4174
npm run dev --workspace=@valverde/admin -- --host 127.0.0.1
```
...
O acompanhamento atual, com etapas concluídas, bloqueios e decisões QA/produção, está em [docs/PLANO_CONFIGURACAO.md](docs/PLANO_CONFIGURACAO.md). Consultar esse plano antes do guia de corte; Vercel e Turnstile estão adiados.
...
O HTML original da gestão permanece em `index.html`. A versão pública recebida é preservada em `legacy/valverde_algarve_2.html` quando se executa `npm run extract:legacy-assets`; as 17 fotografias são extraídas para `apps/site/public/images`.

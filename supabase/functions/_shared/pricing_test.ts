...
const baseFixtures = {
  ...
  seasonal_rates: [{ owner_id: managerId, starts_on: '2026-08-01', ends_on: '2026-08-31', direct_nightly_price: 800, active: true }],
...
Deno.test('preço é calculado no servidor incluindo promoção e piscina aquecida', async () => {
  ...
  if (quote.baseAmount !== 5600 || quote.promoAmount !== 560 || quote.heatedPoolAmount !== 150 || quote.totalAmount !== 5190) {
...
Deno.test('conflitos apenas bloqueiam estadias sobrepostas', async () => {
  ...
  const conflict = await hasReservationConflict(client, { ownerId: managerId, checkIn: '2026-08-22', checkOut: '2026-08-29', nights: 7, baseAmount: 0, heatedPoolAmount: 0, promoAmount: 0, totalAmount: 0, directDiscountPercent: 10, promoDiscountPercent: 0, currency: 'EUR' });
  const adjacent = await hasReservationConflict(fakeClient(baseFixtures), { ownerId: managerId, checkIn: '2026-08-27', checkOut: '2026-09-03', nights: 7, baseAmount: 0, heatedPoolAmount: 0, promoAmount: 0, totalAmount: 0, directDiscountPercent: 10, promoDiscountPercent: 0, currency: 'EUR' });
